...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  Color backgroundColor = const Color.fromARGB(25, 25, 25, 0); // Changed to black background
...
// Display chat history
  ...
  Future<void> _loadPreferences() async {
    ...
    setState(() {
      ...
      backgroundColor = Color(prefs.getInt('backgroundColor') ?? const Color.fromARGB(25, 25, 25, 0).value);
  ...
  void _startWordHighlighting() async {
    ...
    for (int i = 0; i < highlightedWords.length; i++) {
      ...
      // Adjust delay based on speech rate and word length
      final wordLength = highlightedWords[i].length;
      final baseDelay = 200; // Base delay in milliseconds
      final adjustedDelay = (baseDelay + (wordLength * 30)) / speechRate;
      
      await Future.delayed(Duration(milliseconds: adjustedDelay.round()));
    }
  ...
  Widget _buildHighlightedText() {
    if (highlightedWords.isEmpty || !isHighlightingEnabled || !isSpeaking) {
      ...
      String remainingText = outputText;
      int lastIndex = 0;

      for (Match match in boldPattern.allMatches(outputText)) {
        ...
        // Add the bold text
        textSpans.add(TextSpan(
          text: match.group(1), // Get text between **
          style: TextStyle(
            fontSize: fontSize,
            height: lineSpacing,
            letterSpacing: letterSpacing,
        ...
        return Align(
          ...
          child: Container(
            constraints: BoxConstraints(
              maxWidth: 800, // Maximum width for the tab bar
            ),
            child: TabBar(
              ...
              tabAlignment: isCompact ? TabAlignment.fill : TabAlignment.center,
